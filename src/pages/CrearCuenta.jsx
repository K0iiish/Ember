...
// Figma node 955:4655 — star positions mapped to existing asset sizes
// imgStar6 (6×7px) → star-sm.svg
const STARS_SM = [
...
export default function CrearCuenta() {
  ...
  const isActive = nombre.trim().length > 0 && email.trim().length > 0 && password.length > 0
  ...
  return (
    <div className="relative overflow-hidden bg-[#19101b] min-h-screen w-full">
      <style>{`
        .cc-input::placeholder { color: #b3b3b3; opacity: 1; }
        .cc-input { caret-color: #1ad6cf; }
      `}</style>

      {/* Magenta background glow (top-left) */}
      ...
      <div
        ...
        style={{
          top: '315px', left: '18px', right: '18px', height: '56px',
          backgroundColor: '#251b31', borderRadius: '13px',
          border: `1.5px solid ${nombre.length > 0 ? '#1ad6cf' : 'transparent'}`,
        }}
      >
        <span className="absolute left-4" style={{ color: iconColor(nombre.length > 0) }}>
          <IconUser />
        </span>
        <input
          type="text"
          ...
          autoComplete="name"
          className="cc-input w-full h-full bg-transparent focus:outline-none text-white"
      ...
      <div
        className="absolute flex items-center"
        style={{
          top: '487px', left: '18px', right: '18px', height: '56px',
          backgroundColor: '#251b31', borderRadius: '13px',
